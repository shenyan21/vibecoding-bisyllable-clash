...
```powershell
E:\codex_project\hextech-bisyllable-duel
```
...
1. 访问网站主页 `http://www.hextech-bisyllable-duel.com`（或公网 IP 地址）。
2. 在首页输入您的**玩家昵称**。
3. 输入您想加入的**四位数字房间号**（例如 `1234`；或者输入内置测试房号 `TEST`）。
4. 点击 **“创建或加入房间”**，即可即时入场！
5. 成功加入后，可在房间内选择空闲席位准备开战。
...
### 2. 首位创建人即为 Host（先到先得）
* 移除了过往写死的“夏如霜拥有主持人权限”设定。
* **谁先进入新房间（即该房间的第一位创建者/加入者），谁就自动被指定为该房间的“主持人 (Host)”**。
* **主持人自动推选/转接机制**：若当前主持人在游戏中途断开连接或退出房间，**后端会在剩下的在场在线玩家中自动推选第一个人接替成为新的主持人**，避免房间卡死或无法开局。原主持人连回后，会保持原在场身份。
...
### 本地开发
```powershell
cd E:\codex_project\hextech-bisyllable-duel
npm.cmd run dev
```
...
生产实例部署在 Windows 虚拟机，路径为 `C:\apps\hextech-bisyllable-duel`。后台服务通过计划任务 `HextechBisyllableDuel` 管理。

同步最新代码时，只需要覆盖 `src` 与 `server` 目录，随后在服务器 PowerShell 运行以下命令即可：
...
# 验证接口服务
Invoke-WebRequest "http://127.0.0.1/api/public-config" -UseBasicParsing
```
